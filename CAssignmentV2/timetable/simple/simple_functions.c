...
// obviously some wont fit. If we get to the end of all the hours and still have lessons remaining, then the timetable failed
void timetable(Scheme *schemes, int num_of_schemes, Hour *hour, int semester, Lesson **head) {
            ...
            if (schemes[i].num_of_students > 0){ // if the scheme has students on it
                ...
                while (module != NULL) { //for every module
                    if (module->semester == semester) { //if the module takes place in our semester
                        if (module->num_of_practicals > module->num_of_lectures) { //add a practical
                            if (check_for_clashes(&module->clashes, &hour->next, module->module_code) == 0) {
                                ...
                                Lesson *lesson = create_lesson_practical(module->module_code);
                                if (*head == NULL) {
                                    *head = lesson;
                                    hour->next =lesson;
                                    next =*head;
                        ...
                        } else if (module->num_of_lectures > 0) { //add a lecture
                            if (check_for_clashes(&module->clashes, &hour->next, module->module_code) == 0) {
                                ...
                                Lesson *lesson = create_lesson_lecture(module->module_code);
                                if (*head == NULL) {
                                    *head = lesson;
                                    hour->next =lesson;
                                    next =*head;
...
int check_for_clashes(Clash **clash_head, Lesson **lesson_head, char *mc){
...
Lesson *create_lesson_lecture(char *mc){
